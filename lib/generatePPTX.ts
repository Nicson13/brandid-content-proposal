...
export async function generatePPTX(data: ProposalData): Promise<void> {
  ...
  const setBg = (slide: ReturnType<typeof pptx.addSlide>, color = C.paper) => {
  ...
  // Brand name
  s1.addText(data.brandName || 'Brand Name', {
    x: 0.6, y: 1.8, w: 7, h: 1.2,
  ...
  s1.addText(data.industry || '', {
    x: 0.6, y: 3.0, w: 7, h: 0.5,
    fontSize: 16, color: C.accentMid,
    fontFace: 'Arial',
  })

  // Proposal title
  s1.addText(lang === 'zh' ? '社交媒体内容提案' : 'Social Media Proposal', {
    x: 0.6, y: 3.6, w: 7, h: 0.6,
  ...
  // Brandid watermark
  s1.addText('Brandid', {
    x: W - 2.5, y: H - 0.6, w: 2.2, h: 0.4,
    fontSize: 13, color: '444444', align: 'right',
    fontFace: 'Arial',
  })
  ...
  s2.addText(lang === 'zh' ? '项目概览' : 'Overview', {
    x: 0.5, y: 0.4, w: 6, h: 0.6,
    fontSize: 28, bold: true, color: C.ink, fontFace: 'Arial',
  })

  if (data.summary) {
    s2.addText(data.summary, {
      x: 0.5, y: 1.2, w: W - 1, h: 1.6,
      fontSize: 14, color: C.ink, fontFace: 'Arial',
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // Slide 3: 目标与受众 / Goals & Audience
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  const s3 = pptx.addSlide()
  setBg(s3, C.paper)
  s3.addShape('rect', { x: 0, y: 0, w: W, h: 0.06, fill: { color: C.accent } })

  s3.addText(lang === 'zh' ? '营销目标 & 受众画像' : 'Goals & Audience', {
  ...
  goals.forEach((goal, i) => {
    const x = 0.5 + i * 6.4
    s3.addShape('rect', { x, y: 1.2, w: 6.0, h: 1.4, fill: { color: C.accent }, line: { color: C.accent } })
    s3.addText(lang === 'zh' ? `目标 ${i + 1}` : `Goal ${i + 1}`, {
      x: x + 0.2, y: 1.3, w: 5.6, h: 0.4, fontSize: 11, color: C.accentLight, fontFace: 'Arial',
    })
    s3.addText(goal || '', {
      x: x + 0.2, y: 1.7, w: 5.6, h: 0.7, fontSize: 15, bold: true, color: C.white, fontFace: 'Arial', wrap: true,
    })
  })
  ...
  s3.addText(lang === 'zh' ? '内容配比' : 'Content Mix', {
    x: 0.5, y: 2.9, w: 4, h: 0.4, fontSize: 14, bold: true, color: C.ink, fontFace: 'Arial',
  })
  ...
  if (data.personas) {
    ...
    s3.addText(lang === 'zh' ? '目标受众' : 'Target Audience', {
      x: 0.7, y: 5.7, w: 4, h: 0.35, fontSize: 11, color: C.muted, fontFace: 'Arial',
    })
    s3.addText(data.personas, {
      x: 0.7, y: 6.05, w: W - 1.4, h: 0.9, fontSize: 12, color: C.ink, wrap: true, fontFace: 'Arial',
    })
  }

  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // Slide 4+: 选题卡片（每张 slide 2个 topic）
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  for (let si = 0; si < Math.ceil(topics.length / perSlide); si++) {
    ...
    const pageLabel = lang === 'zh'
      ? `选题方向 ${sliceStart + 1}–${sliceStart + pageTopics.length}`
      : `Topics ${sliceStart + 1}–${sliceStart + pageTopics.length}`

    s.addText(pageLabel, {
      x: 0.5, y: 0.25, w: 6, h: 0.5,
      fontSize: 22, bold: true, color: C.ink, fontFace: 'Arial',
    })

    pageTopics.forEach((topic, ti) => {
      ...
      s.addShape('rect', {
        ...
        shadow: { type: 'outer', blur: 8, offset: 2, angle: 45, color: '00000015' }
      })
      ...
      // Index
      s.addText(`#${topic.id || (sliceStart + ti + 1)}`, {
        x: cardX + 0.2, y: cardY + 0.2, w: 1, h: 0.4,
      ...
      s.addText(topic.goal || '', {
        x: cardX + 1.0, y: cardY + 0.2, w: 1.4, h: 0.36,
        fontSize: 10, bold: true, color: gColor, align: 'center', valign: 'middle', fontFace: 'Arial',
      })
      ...
      s.addText(topic.format || '', {
        x: cardX + 2.6, y: cardY + 0.2, w: 2.0, h: 0.36,
        fontSize: 10, bold: true, color: fColor, align: 'center', valign: 'middle', fontFace: 'Arial',
      })

      // Title
      s.addText(topic.title || '', {
        x: cardX + 0.25, y: cardY + 0.72, w: cardW - 0.5, h: 0.9,
        fontSize: 16, bold: true, color: C.ink, wrap: true, valign: 'top', fontFace: 'Arial',
      })
      ...
      // Audience
      s.addText(lang === 'zh' ? '受众' : 'Audience', {
        x: cardX + 0.25, y: cardY + 1.85, w: cardW - 0.5, h: 0.3,
        fontSize: 9, color: C.muted, fontFace: 'Arial',
      })
      s.addText(topic.audience || '', {
        x: cardX + 0.25, y: cardY + 2.15, w: cardW - 0.5, h: 0.5,
        fontSize: 11, color: C.ink, wrap: true, fontFace: 'Arial',
      })

      // Hook
      s.addText(lang === 'zh' ? '开场钩子' : 'Hook', {
        x: cardX + 0.25, y: cardY + 2.75, w: cardW - 0.5, h: 0.3,
        fontSize: 9, color: C.muted, fontFace: 'Arial',
      })
      ...
      // Caption preview
      s.addText(lang === 'zh' ? '文案参考' : 'Caption', {
        x: cardX + 0.25, y: cardY + 4.0, w: cardW - 0.5, h: 0.3,
        fontSize: 9, color: C.muted, fontFace: 'Arial',
      })
      const captionText = (topic.caption || '').length > 180
        ? (topic.caption || '').substring(0, 180) + '…'
        : (topic.caption || '')
  ...
  if (data.toneNote || data.styleNote) {
    ...
    sT.addText(lang === 'zh' ? '品牌口吻指南' : 'Tone of Voice', {
      x: 0.5, y: 0.8, w: W - 1, h: 0.8,
      fontSize: 34, bold: true, color: C.white, fontFace: 'Arial',
    })

    if (data.toneNote) {
      sT.addText(data.toneNote, {
        x: 0.5, y: 1.9, w: W - 1, h: 2.5,
        fontSize: 16, color: 'DDDDDD', wrap: true, valign: 'top', fontFace: 'Arial',
      })
    }

    if (data.styleNote) {
      sT.addShape('rect', { x: 0.5, y: 4.6, w: W - 1, h: 1.6, fill: { color: '222222' } })
      sT.addText(lang === 'zh' ? '风格备注' : 'Style Notes', {
        x: 0.7, y: 4.7, w: 4, h: 0.4, fontSize: 11, color: C.muted, fontFace: 'Arial',
      })
      sT.addText(data.styleNote, {
        x: 0.7, y: 5.1, w: W - 1.4, h: 0.9, fontSize: 13, color: 'BBBBBB', wrap: true, fontFace: 'Arial',
      })
    }
  }
  ...
  sLast.addText(lang === 'zh' ? '下一步行动' : 'Next Steps', {
    x: 0.5, y: 0.4, w: 8, h: 0.7,
    fontSize: 32, bold: true, color: C.ink, fontFace: 'Arial',
  })
  ...
  steps.forEach((step, i) => {
    ...
    sLast.addText(String(i + 1), {
      x: 0.5, y, w: 0.7, h: 0.7, fontSize: 18, bold: true, color: C.white, align: 'center', valign: 'middle', fontFace: 'Arial',
    })
    sLast.addText(step, {
      x: 1.4, y: y + 0.05, w: W - 2, h: 0.6, fontSize: 16, color: C.ink, fontFace: 'Arial', wrap: true,
    })
  })

  // Contact / footer
  sLast.addShape('rect', { x: 0.5, y: H - 1.2, w: W - 1, h: 0.9, fill: { color: C.accentLight } })
  sLast.addText(lang === 'zh' ? '感谢信任 · Brandid' : 'Thank you · Brandid', {
    x: 0.7, y: H - 1.15, w: W - 1.4, h: 0.4, fontSize: 13, bold: true, color: C.accent, fontFace: 'Arial',
  })
  sLast.addText('www.brandid.com', {
    x: 0.7, y: H - 0.8, w: W - 1.4, h: 0.35, fontSize: 12, color: C.muted, fontFace: 'Arial',
  })
